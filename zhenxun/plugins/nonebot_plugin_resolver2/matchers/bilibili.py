...
async def _(text: str = ExtractText(), keyword: str = Keyword()):
    ...
    # 视频
    if keyword in ("av", "/av"):
        video_info = await parser.parse_video_info(
            avid=int(video_id), page_num=page_num
        )
    else:
        ...
        if video_info.audio_url:
            v_path, a_path = await asyncio.gather(
                download_file_by_stream(
                    ...
                    file_name=f"{file_name}-video.m4s",
                    ext_headers=parser.headers,
                ),
                download_file_by_stream(
                    video_info.audio_url,
                    file_name=f"{file_name}-audio.m4s",
                    ext_headers=parser.headers,
                ),
            )
