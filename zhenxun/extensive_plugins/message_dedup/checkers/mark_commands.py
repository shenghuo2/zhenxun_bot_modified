...
from ..utils import (compute_sha256, contains_bilibili_url,
                     extract_bilibili_card_info, extract_bilibili_url,
                     int_to_datetime, resolve_bilibili_video_id)
from . import build_dup_reply, check_or_store
from .bilibili_checker import make_video_sha
...
async def handle_mark(event: MessageEvent, bot: Bot, session: Uninfo):
    ...
    if card_info:
        ...
        result = await check_or_store(sha, group_id, replied_id, sender_id, reply_ts)
        if result.is_dup:
            await mark_cmd.finish(build_dup_reply(
                result, event.user_id,
                f"视频《{video_title}》在{result.time_diff_str}就有人标记过了，还标记，杂鱼~（第{result.hit_count}次发了捏）",
            ))
        else:
    ...
    if result.is_dup:
        await mark_cmd.finish(build_dup_reply(
            result, event.user_id,
            f"该图片在{result.time_diff_str}就有人标记过了，还标记，杂鱼~（第{result.hit_count}次发了捏）",
        ))
    else:
