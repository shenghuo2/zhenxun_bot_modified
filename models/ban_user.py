...
class BanUser(Model):
    ...
    @classmethod
    async def check_ban_level(cls, user_id: str, level: int) -> bool:
        """
        说明:
    ...
    @classmethod
    async def check_ban_time(cls, user_id: str) -> Union[str, int]:
        """
        说明:
            检测用户被ban时长
        参数:
            :param user_id: 用户id
        """
        logger.debug(f"获取用户ban时长", target=user_id)
    ...
    async def is_ban(cls, user_id: str) -> bool:
        ...
        参数:
            :param user_id: 用户id
        """
        logger.debug(f"检测是否被ban", target=user_id)
    ...
    @classmethod
    async def is_super_ban(cls, user_id: str) -> bool:
        """
        说明:
            判断用户是否被超级用户ban / b了
        参数:
            :param user_id: 用户id
        """
        logger.debug(f"检测是否被超级用户权限封禁", target=user_id)
        if user := await cls.filter(user_id=user_id).first():
    ...
    @classmethod
    async def ban(cls, user_id: str, ban_level: int, duration: int):
        """
        说明:
            ban掉目标用户
        参数:
        ...
        """
        logger.debug(f"封禁用户，等级:{ban_level}，时长: {duration}", target=user_id)
        if await cls.filter(user_id=user_id).first():
            await cls.unban(user_id)
        await cls.create(
            user_id=user_id,
            ban_level=ban_level,
    ...
    async def unban(cls, user_id: str) -> bool:
        ...
        """
        if user := await cls.filter(user_id=user_id).first():
            logger.debug("解除封禁", target=user_id)
            await user.delete()
            return True
        return False
