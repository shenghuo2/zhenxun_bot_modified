...
class GroupInfoUser(Model):
    ...
    @classmethod
    async def set_user_nickname(cls, user_id: str, group_id: str, nickname: str):
        """
        说明:
            设置群员在该群内的昵称
        参数:
    ...
    @classmethod
    async def get_group_member_uid(cls, user_id: str, group_id: str) -> Optional[int]:
        logger.debug(
        ...
        if not user.uid:
            all_user = await cls.filter(user_id=user_id).all()
            for x in all_user:
                if x.uid:
                    return x.uid
            user.uid = _max_uid + 1
